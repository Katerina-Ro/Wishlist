...
public class ButtonClick {
    ...
    @Autowired
    public ButtonClick(@Qualifier("infoCommand") Command infoCommand, @Qualifier("addCommand")Command addCommand, @Qualifier("changeWishCommand")
    ...
    public EditMessageText getCommandResponse (String message, long chat_id, long message_id) throws TelegramApiException {
        if (message.equals(COMMANDS.INFO.getCommand())) {
            return infoCommand.execute(chat_id, message_id);
        }
        if (message.equalsIgnoreCase(COMMANDS.ADD_WISH.getCommand())) {
            return addCommand.execute(chat_id, message_id);
        }
        if (message.equalsIgnoreCase(COMMANDS.CHANGE_WISH.getCommand())) {
            return changeWishCommand.execute(chat_id, message_id);
        }
        return getWishList.execute(chat_id, message_id);
    }
}
