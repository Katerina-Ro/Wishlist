        ...
        if (true){
            ...
            if (gift.getStatusGiftAdmin() != STATUS_GIFT.NOT_ACTIVE) {
                if (gift.getStatusGiftOwn() == STATUS_GIFT.ACTIVE) {
                    ...
                    System.out.println("Ваше пожелание стало " + STATUS_GIFT.NOT_ACTIVE.getStatusGift() + ". Теперь никто, кроме Вас его не видит");
                } else {
                    gift.setStatusGiftOwn(STATUS_GIFT.ACTIVE);
                    System.out.println("Ваше пожелание стало " + STATUS_GIFT.ACTIVE.getStatusGift() + ". Теперь его могут видеть все");
                }
    ...
    public String changeStatusGiftAnother(Gift gift) {
        ...
        if (true){
            ...
            if (gift.getStatusGiftOwn().equals(STATUS_GIFT.ACTIVE) && gift.getStatusGiftAdmin()
                    .equals(STATUS_GIFT.ACTIVE) && gift.getStatusGiftAnother(STATUS_GIFT.NOT_ACTIVE).equals(STATUS_GIFT.NOT_ACTIVE))
            {
